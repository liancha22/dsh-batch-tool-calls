...
- 它是**软约束**，不是闸门：不拦工具、不设权限、不改工具 schema 与并发上限
  （`maxParallelToolCalls` 仍是宿主默认 10），也不要求任何模式为它做适配 —— 它影响的是「倾向」，
  不是「能不能」。所以标准 / PTC / 极简 / plan 模式、子代理（`subagent` / `subagent_fork`）、
  workflow 派生的 Agent 都可以一直留着它，不会让哪个模式失效，也不会让某个模式变强。
...
node scripts/cache-audit.mjs <会话日志>    # 逐请求看提示缓存：命中率、每次「缓存中断」是被什么打断的
```

`cache-audit.mjs` 会把每个请求的未命中/命中读/写入、以及每次「缓存中断」（本次命中读不足
上次前缀的一半）前面刚发生了什么列出来，并按原因汇总。判断某个改动是不是把缓存搞坏了，
看这个就够了 —— 别靠感觉。
...
- **「它是不是只对某个模式生效 / 会不会跟别的模式冲突」**：**全模式可用，也不冲突**。
  它不拦工具、不设权限、不加 hook，只是系统提示里的一段文字 + 压缩后那条提醒；
  注册在**宿主组合**（`dsh --profile web --dump-config` 里是顶层行 `- id: batch-tool-calls`，
  不在任何预设内部），所以换模式、进子代理、跑 workflow 都照样生效，装一次就够。
  预设想换措辞，注册同名 section `batch-tool-calls:policy` 覆盖即可（scoped 覆盖 global）。
- **「有 `~/.dsh/AGENTS.md` 就够了」**：那是「工作区指令」，属于软提示、按会话注入；
  ...
  压缩后那一条贴着当前步骤的短提醒。
- **「装了它会把提示缓存搞坏」**：不会，而且可以量出来。用 `scripts/cache-audit.mjs` 读真实会话：
  - 插进去的消息永远是**追加在对话末尾**的，前面那一整段前缀没变 → 该命中的照旧命中；
  - 那条提醒**只在压缩那一步出现**（因为触发条件就是 `compaction/end`），而压缩本身就把
    旧历史换成了 checkpoint、前缀必然重算 —— 也就是说不装本插件，这一次照样要重算。
    实测两个会话里「只有提醒、没有压缩」的请求都是 **0 次**；
  - 真实数字（同一台机器、本插件已加载的两个会话）：
    会话 A 整体命中率 **99.4%**，全程只有 **3 次**缓存中断，原因**全部**是压缩本身
    （未命中 17.7k / 6.1k / 6.0k）；
    会话 B 整体 **95.2%**，**11 次**中断按原因归为：工具清单变化 **8 次**（合计 1.41M 未命中，
    单次最高 **323k**）、进程重启 1 次（196k）、压缩 2 次（合计 22k）。
    两个会话里本插件那条提醒都与压缩同批出现，「只有提醒、没有压缩」的请求 **0 次**。
  - 真正贵的是**工具清单变化**（动态注册/注销工具会让 `request/header` 重新提交，整条前缀重算），
    这一类才是该盯的地方。
  - 本插件对缓存的全部代价就是提示段那 513 字符（约 350 token，进前缀后同样按缓存价计费），
    加上每次压缩后约 60 token 的提醒。
...
- 新增 `scripts/cache-audit.mjs`：逐请求审计提示缓存，把每次「缓存中断」和它前面刚发生的事件
  （压缩 / 重启 / 模型切换 / 工具清单变化 / 系统提示重新提交）对起来，并按原因汇总。
- README 补上「装了它会不会把缓存搞坏」的实测口径。运行时行为与 1.1.1 相同。
